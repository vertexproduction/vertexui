--[[
	...
	Vertex Interface Suite
	(Revamped from Rayfield by Sirius)
	
	Credits to the original Rayfield team:
...
if _getgenv then
	local ok, result = pcall(function() return _getgenv().DISABLE_VERTEX_REQUESTS end)
	if ok and result then requestsDisabled = true end
	local ok2, result2 = pcall(function() return _getgenv().VERTEX_ASSET_ID end)
	if ok2 and type(result2) == "number" then customAssetId = result2 end
	local ok3, result3 = pcall(function() return _getgenv().VERTEX_SECURE end)
...
local function secureNotify(wType, title, content)
	...
	task.spawn(function()
		while not VertexLibrary or not VertexLibrary.Notify do task.wait(0.5) end
		VertexLibrary:Notify({
			Title = title,
			Content = content,
			Duration = 8,
		})
...
local InterfaceBuild = 'VERTEX_1'
local Release = "Vertex 1.0"
local VertexFolder = "Vertex"
local ConfigurationFolder = VertexFolder.."/Configurations"
local ConfigurationExtension = ".vtxcfg"
local settingsTable = {
	General = {
		vertexOpen = {Type = 'bind', Value = 'K', Name = 'Vertex Keybind'},
	},
	System = {
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.vertexOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("Vertex | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		if callSafely(isfolder, VertexFolder) then
			if callSafely(isfile, VertexFolder..'/settings'..ConfigurationExtension) then
				file = callSafely(readfile, VertexFolder..'/settings'..ConfigurationExtension)
			end
		end
	...
	{"General":{"vertexOpen":{"Value":"K","Type":"bind","Name":"Vertex Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Vertex Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
		...
		-- If no settings saved, apply overridden settings only
			...
			for settingName, settingValue in overriddenSettings do
				...
				assert(#split == 2, "Vertex | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('Vertex had an issue accessing configuration saving capability.')
		end
	end
end
...
local VertexAssetId = customAssetId or 10804731440
local VertexUI = useStudio and script.Parent:FindFirstChild('Vertex') or game:GetObjects("rbxassetid://"..VertexAssetId)[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local vertexDestroyed = false -- True when VertexLibrary:Destroy() is called

repeat
	if VertexUI:FindFirstChild('Build') and VertexUI.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Vertex | Build Mismatch')
		print('Vertex may encounter issues as you are running an incompatible interface version ('.. ((VertexUI:FindFirstChild('Build') and VertexUI.Build.Value) or 'No Build') ..').\n\nThis version of Vertex is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
	toDestroy, VertexUI = VertexUI, useStudio and script.Parent:FindFirstChild('Vertex') or game:GetObjects("rbxassetid://"..VertexAssetId)[1]
...
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	VertexUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == VertexUI.Name and Interface ~= VertexUI then
			Interface.Enabled = false
			Interface.Name = "Vertex-Old"
		end
	end
elseif not useStudio then
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default Vertex asset ID. Set VERTEX_ASSET_ID to a custom upload to avoid detection.")
end

do
	local AssetPath = VertexFolder.."/Assets"
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("Vertex | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("Vertex | Failed to load custom assets: "..tostring(err))
			secureNotify("asset_load_fail", "Vertex", "Failed to load custom assets. UI images may not display correctly.")
		end
	else
		secureNotify("no_getcustomasset", "Vertex", "Your executor does not support getcustomasset. Some UI images may not render correctly.")
	end

	VertexUI.Main.Shadow.Image.Image = customAssets[tostring(5587865193)]
	VertexUI.Main.Topbar.Hide.Image = customAssets[tostring(10137832201)]
	VertexUI.Main.Topbar.ChangeSize.Image = customAssets[tostring(10137941941)]
	VertexUI.Main.Topbar.Settings.Image = customAssets[tostring(80503127983237)]
	VertexUI.Main.Topbar.Icon.Image = customAssets[tostring(78137979054938)]
	VertexUI.Main.Topbar.Search.Image = customAssets["IconMagnifyingGlass2"]
	VertexUI.Main.Topbar.Search.ImageRectOffset = Vector2.new(0, 0)
	VertexUI.Main.Topbar.Search.ImageRectSize = Vector2.new(0, 0)
	VertexUI.Main.Elements.Template.Toggle.Switch.Shadow.Image = customAssets[tostring(3602733521)]
	VertexUI.Main.Elements.Template.Slider.Main.Shadow.Image = customAssets[tostring(3602733521)]
	VertexUI.Main.Elements.Template.Dropdown.Toggle.Image = customAssets["IconChevronTopMedium"]
	VertexUI.Main.Elements.Template.Dropdown.Toggle.ImageRectOffset = Vector2.new(0, 0)
	VertexUI.Main.Elements.Template.Dropdown.Toggle.ImageRectSize = Vector2.new(0, 0)
	VertexUI.Main.Elements.Template.Label.Icon.Image = customAssets[tostring(11745872910)]
	VertexUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.Image = customAssets[tostring(11413591840)]
	VertexUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.MainPoint.Image = customAssets[tostring(3259050989)]
	VertexUI.Main.Elements.Template.ColorPicker.ColorSlider.SliderPoint.Image = customAssets[tostring(3259050989)]
	VertexUI.Main.TabList.Template.Image.Image = customAssets[tostring(4483362458)]
	VertexUI.Main.Search.Search.Image = customAssets[tostring(18458939117)]
	VertexUI.Main.Search.Shadow.Image = customAssets[tostring(5587865193)]
	VertexUI.Notifications.Template.Icon.Image = customAssets[tostring(77891951053543)]
	VertexUI.Notifications.Template.Shadow.Image = customAssets[tostring(3523728077)]
	VertexUI.Loading.Banner.Image = customAssets[tostring(111263549366178)]
...
if VertexUI.AbsoluteSize.X < minSize.X and VertexUI.AbsoluteSize.Y < minSize.Y then
...
local MPrompt = VertexUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = VertexUI:FindFirstChild('Drag')
...
local keybindConnections = {} -- For storing keybind connections to disconnect when Vertex is destroyed

local SelectedTheme = VertexLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = VertexLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	VertexUI.Main.BackgroundColor3 = SelectedTheme.Background
	VertexUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	VertexUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	VertexUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	VertexUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	VertexUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	VertexUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		VertexUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		VertexUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(VertexUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Vertex | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Vertex | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Vertex had an issue decoding the configuration file, please try delete the file and reopen Vertex.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(VertexLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Vertex | Unable to find '"..FlagName.. "' in the save file.")
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(VertexLibrary.Flags) do
...
function VertexLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Vertex | Not seeing your actions in notifications?')
			print("Notification Actions are being sunset for now, keep up to date on when they're back.")
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			VertexLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			VertexLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "vertexOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, VertexFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Vertex Settings', 0, true)

	if TabList['Vertex Settings'] then
		TabList['Vertex Settings'].LayoutOrder = 1000
	end

	if Elements['Vertex Settings'] then
		Elements['Vertex Settings'].LayoutOrder = 1000
	end
...
function VertexLibrary:CreateWindow(Settings)
	if VertexUI:FindFirstChild('Loading') then
		if getgenv and not getgenv().vertexCached then
			VertexUI.Enabled = true
			VertexUI.Loading.Visible = true

			task.wait(1.4)
			VertexUI.Loading.Visible = false
		end
	end

	if getgenv then getgenv().vertexCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				VertexLibrary:Notify({Title = 'Build Mismatch', Content = 'Vertex may encounter issues as you are running an incompatible interface version ('.. ((VertexUI:FindFirstChild('Build') and VertexUI.Build.Value) or 'No Build') ..').\n\nThis version of Vertex is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Vertex"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Vertex Interface Suite" then
		LoadingFrame.Version.Text = "Vertex UI"
	end
	...
	if not Settings.DisableVertexPrompts then
		task.spawn(function()
			while not vertexDestroyed do
				task.wait(math.random(180, 600))
				if vertexDestroyed then break end
				VertexLibrary:Notify({
					Title = "Vertex Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		ensureFolder(VertexFolder.."/Discord Invites")

		if not callSafely(isfile, VertexFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, VertexFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Vertex RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(VertexFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Vertex | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Vertex specific development.')
				end
			end
		end
		...
		if callSafely(isfile, VertexFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, VertexFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
		...
		if not Passthrough and secureMode then
			warn("Vertex | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
			VertexUI.Enabled = false
			return VertexLibrary
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, VertexFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						VertexLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	if Settings.KeySystem then
		...
		if vertexDestroyed then return end
	end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Vertex | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Vertex specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(MainCP, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= VertexLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			MainCP.MouseButton1Down:Connect(function()
				if opened then
					mainDragging = true 
				end
			end)
			MainCP.MainPoint.MouseButton1Down:Connect(function()
				if opened then
					mainDragging = true 
				end
			end)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					VertexLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			VertexUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
